...
// TODO : Translate this whole file into C++. and make an API for it
...
impl DebugInfo {
    ...
    pub fn add_type<S: BnStrCompatible>(self, name: S, new_type: Ref<Type>) -> bool {
        let name = name.as_bytes_with_nul();
        unsafe {
            BNAddDebugType(
                self.handle,
    ...
    pub fn add_function<S: BnStrCompatible>(self, new_func: DebugFunctionInfo<S>) -> bool {
        ...
        let mut parameter_types: Vec<*mut BNType> =
            new_func.parameters.iter().map(|(_, t)| t.handle).collect();

        // TODO : References to temporaries?
        let parameter_names: Vec<S> = new_func.parameters.into_iter().map(|(n, _)| n).collect();
